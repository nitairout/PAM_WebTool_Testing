...
/*
 * This test verifies if the data is corrected as expected for a site when there is a Null value followed by Catch-up spike 
 * (there is a time stamp but value is missing)
 * */
public class QIDM_161_NullWithCatchUpSpike extends TestBase {
	LoginTC login = null;
	@Test
	public void nullWithCatchUpSpike() throws Throwable {
		try {
			...
			//Select Electric commodity for a site NullWithCatchUpSpike
			searchSiteInLocationList("NullWithCatchUpSpike");
			getWebElementXpath("DQElectricitySiteAfterFilter").click();
			Reporter.log("Searched for 'NullWithCatchUpSpike' site");
			...
			//Legend verification
			Assert.assertEquals(getWebElementXpath("LineLegendOne").getText(), "QA Site 1 \\...\\ NullWithCatchUpSpike kWh");
			...
			aJaxWait();
			Assert.assertEquals(getWebElementActionXpath("ChartFlagPopUpHeaderSiteName").getText(), "QA Site 1 \\...\\ NullWithCatchUpSpike");
			//String rawexceptionNote="[DeltaCalculator 2020-01-17 13:50:21] System was unable to calculate the previous 5 values due to Data Error | System detected a catch-up spike.";
			String rawexceptionNote="System was unable to calculate the previous 5 values due to Data Error | System detected a catch-up spike.";
			...
			Assert.assertEquals(getWebElementXpath_D("//*[@id='tablehead']/tr[1]/th[2]").getText(), "QA Site 1 \\ Z_Data Quality Scenarios \\ NullWithCatchUpSpike");
			...
			String rawModeTableData[]="7/5/2019 09:15 PM~~Unknown|7/5/2019 09:30 PM~~Unknown|7/5/2019 09:45 PM~~Unknown|7/5/2019 10:00 PM~~Unknown|7/5/2019 10:15 PM~~Unknown|7/5/2019 10:30 PM~600.0~Suspect+Note".split("\\|");
			...
			Assert.assertEquals(iconToolTip, "This chart contains 6 estimated values.");
			...
			getWebElementActionXpath_D("("+locators.getProperty("DataQualityFlagsOnCorectMode")+")[6]").click();
			aJaxWait();
			Assert.assertEquals(getWebElementActionXpath("ChartFlagPopUpHeaderSiteName").getText(), "QA Site 1 \\...\\ NullWithCatchUpSpike");
			
			//Verified 1st note i.e. 1st click (Fri - Jul 5, 2019 - 22:30:00) 
			//Assert.assertEquals(getWebElementXpath("ChartFlagPopupNote").getText().replace("\n", " "), "[DeltaCalculator 2020-01-17 13:50:21] System was unable to calculate the previous 5 values due to Data Error | System detected a catch-up spike. Estimate calculated with weighted linear interpolation.");
			Assert.assertTrue(getWebElementXpath("ChartFlagPopupNote").getText().replace("\n", " ").contains("System was unable to calculate the previous 5 values due to Data Error | System detected a catch-up spike. Estimate calculated with weighted linear interpolation."));
			for(int i=0;i<5;i++) {
				getWebElementActionXpath_D("//span[contains(text(),'Prev')]").click();
				Thread.sleep(1000);
				Assert.assertEquals(getWebElementXpath("ChartFlagPopupNote").getText(), "Estimate calculated with weighted linear interpolation.");
			}
			...
			//Table Header verification
			Assert.assertEquals(getWebElementActionXpath_D("//*[@id='tablehead']/tr[1]/th[2]").getText(), "QA Site 1 \\ Z_Data Quality Scenarios \\ NullWithCatchUpSpike");
			...
			movingTill=34;
			rowVal=i;
			String correctedModeTableData[]="7/5/2019 09:15 PM~100.0~Valid+Note+Estimate|7/5/2019 09:30 PM~100.0~Valid+Note+Estimate|7/5/2019 09:45 PM~100.0~Valid+Note+Estimate|7/5/2019 10:00 PM~100.0~Valid+Note+Estimate|7/5/2019 10:15 PM~100.0~Valid+Note+Estimate|7/5/2019 10:30 PM~100.0~Valid+Note+Estimate".split("\\|");
