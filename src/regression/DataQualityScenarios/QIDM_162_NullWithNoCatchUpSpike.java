 ...
 * */
public class QIDM_162_NullWithNoCatchUpSpike extends TestBase {
	LoginTC login = null;
	@Test
	public void nullWithNoCatchUpSpike() throws Throwable {
		try {
			...
			//Select Electric commodity for a site NullWithNoCatchUpSpike
			searchSiteInLocationList("NullWithNoCatchUpSpike");
			getWebElementXpath("DQElectricitySiteAfterFilter").click();
			Reporter.log("Searched for 'NullWithNoCatchUpSpike' site");
			...
			//Legend verification
			Assert.assertEquals(getWebElementXpath("LineLegendOne").getText(), "QA Site 1 \\...\\ NullWithNoCatchUpSpike kWh");
			...
			aJaxWait();
			Assert.assertEquals(getWebElementActionXpath("ChartFlagPopUpHeaderSiteName").getText(), "QA Site 1 \\...\\ NullWithNoCatchUpSpike");
			//String rawexceptionNote="[DeltaCalculator 2020-01-17 13:50:22] System was unable to calculate the previous 5 values due to Data Error | System did not detect a catch-up spike.";
			String rawexceptionNote="System was unable to calculate the previous 5 values due to Data Error | System did not detect a catch-up spike.";
			Assert.assertTrue(getWebElementXpath("ChartFlagPopupNote").getText().replace("\n", " ").contains(rawexceptionNote));//Fri - Jul 5, 2019 - 22:30:00
			...
			Assert.assertEquals(getWebElementXpath_D("//*[@id='tablehead']/tr[1]/th[2]").getText(), "QA Site 1 \\ Z_Data Quality Scenarios \\ NullWithNoCatchUpSpike");
			...
			//Assert.assertEquals(getWebElementXpath("ChartFlagPopupNote").getText().replace("\n", " "), "[DeltaCalculator 2020-01-17 13:50:22] System was unable to calculate the previous 5 values due to Data Error | System did not detect a catch-up spike.");
			...
			//Table Header verification
			Assert.assertEquals(getWebElementActionXpath_D("//*[@id='tablehead']/tr[1]/th[2]").getText(), "QA Site 1 \\ Z_Data Quality Scenarios \\ NullWithNoCatchUpSpike");
