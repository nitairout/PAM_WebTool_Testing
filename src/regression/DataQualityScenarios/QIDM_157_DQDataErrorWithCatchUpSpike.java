...
/*
 * This test verifies if the data is corrected as expected for a siter when there is a Data Error with Catch-up spike 
 * (Bad value identified by source system (Ex: Com’X code 19)).
 * */
public class QIDM_157_DQDataErrorWithCatchUpSpike extends TestBase {
	LoginTC login = null;
	@Test
	public void dqDataErrorWithCatchUpSpike() throws Throwable {
		try {
			...
			searchSiteInLocationList("DataErrorWithCatchUpSpike");
			getWebElementXpath("DQElectricitySiteAfterFilter").click();
			Reporter.log("Searched for 'DataErrorWithCatchUpSpike' site");
			...
			//Legend verification
			Assert.assertEquals(getWebElementXpath("LineLegendOne").getText(), "QA Site 1 \\...\\ DataErrorWithCatchUpSpike kWh");
			...
			Thread.sleep(1000);
			Assert.assertEquals(getWebElementActionXpath("ChartFlagPopUpHeaderSiteName").getText(), "QA Site 1 \\...\\ DataErrorWithCatchUpSpike");
			...
			Assert.assertEquals(getWebElementXpath_D("//*[@id='tablehead']/tr[1]/th[2]").getText(), "QA Site 1 \\ Z_Data Quality Scenarios \\ DataErrorWithCatchUpSpike");
			...
			//This is added because the DQ flags were overlapped
			// Select the Fixed date range of July052019 and July062019
			...
			//Assert.assertEquals(getWebElementXpath("ChartFlagPopupNote").getText().replace("\n", " "), "[DeltaCalculator 2020-01-17 13:50:19] System was unable to calculate the previous 5 values due to Data Error | System detected a catch-up spike. Estimate calculated with weighted linear interpolation.");
			...
			//Table Header verification
			Assert.assertEquals(getWebElementActionXpath_D("//*[@id='tablehead']/tr[1]/th[2]").getText(), "QA Site 1 \\ Z_Data Quality Scenarios \\ DataErrorWithCatchUpSpike");
			...
			if(checkBrowserHeadless) {
				...
				//Moving the cursor position to the table data where we have the test data to verify
				Utility.moveTheElement("//table[@id='maintable']//tr[55]");
				Thread.sleep(2000);
				Utility.moveTheElement("//table[@id='maintable']//tr[25]");
				Thread.sleep(2000);
				i=29;
				movingTill=34;
				rowVal=i;
