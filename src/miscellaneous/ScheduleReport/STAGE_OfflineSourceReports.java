...
public class STAGE_OfflineSourceReports extends TestBase{
	@Test(retryAnalyzer = NoRetry.class)
	public void Stage_offLineDailyReports() throws Exception
	{
			try {
				...
				*/
				...
				for(int i=1;i<3;i++) {
					...
					if(emails.contains("Stage_Offline Source Report")  ) {
							if(emails.contains(executionTodayDate)) {
								...
								printLog("There is no Stage Offline report generated today.;");
								throw new Exception("Seems like there is no Stage Offline report generated today");
							}
							break;
					}
				}
