...
public class PAMDailyScheduledReports extends TestBase{
int stageCounter=1;
int prodCounter=1;
@Test(retryAnalyzer = NoRetry.class)
	public void intervalDataDailyReports() throws Exception
	{
			try {
			...
			getWebElementXpath_D("//input[contains(@name,'Passwd')]").sendKeys("pamtest!@#");
			...
			int stage_envExecution=0;
			int prod_envExecution=0;
			int stage_scheduled=0;
			int prod_scheduled=0;
			...
			for(int i=1;i<3;i++) {
				...
				if(emails.contains("Auto_Stage")) {
					try {
						...
						stage_envExecution=1;
					}finally {
						if(stageCounter==1)
							d.navigate().back();
					}
				}
				if(emails.contains("Auto_Prod")  ) {
					try {
						if(emails.contains(executionTodayDate)) {
							//d.findElement(By.xpath("//table/tbody/tr["+i+"]/td[5]/div/div/div/span/span[contains(@class,'bqe')]")).click();
							getWebElementActionXpath_D("((//div[contains(@class,'bGI nH')])[2]//td[@class='xY a4W']//div[@class='xT'])["+i+"]//span[@class='bqe']").click();
							Thread.sleep(4000);
							prodScheduleVerification(executionDate);
							d.navigate().back();
							Thread.sleep(4000);
							prodCounter=0;
						}else {
			...
			if(stage_envExecution==1 && prod_envExecution==1) {
					printLog("There is some issue with stage report generation");
					printLog("There is some issue with prod report generation");
					throw new Exception("prod_envExecution & stage_envExecution =1");
			}else {
				if(stage_envExecution==1) {
					printLog("There is some issue with stage report generation");
					throw new Exception("stage_envExecution =1");
				}
				if(prod_envExecution==1) {
					printLog("There is some issue with prod report generation");
					throw new Exception("prod_envExecution =1");
				}
			}
			if(stage_scheduled==1 && prod_scheduled==1) {
					printLog("Stage report did not generated today!!");
					printLog("Prod report did not generated today!!");
					throw new Exception("prod_scheduled & stage_scheduled =1");
			}else {
				if(stage_scheduled==1) {
					printLog("Stage report did not generated today!!");
					throw new Exception("stage_scheduled =1");
				}
				if(prod_scheduled==1) {
					printLog("Prod report did not generated today!!");
					throw new Exception("prod_scheduled =1");
				}
			}
	...
	public void prodScheduleVerification(String executionDate) throws Exception {
		try {
			printLog("");	
			printLog("*******Prod schedule report verification started!!*******");
			printLog("");	
				//number of excel attachment verification
				int numberOfExcel=d.findElements(By.xpath(locators.getProperty("checkattachement"))).size();
				if(numberOfExcel==2) {
				...
				//all report name declared
				List<String> reportHeadersArray = Arrays.asList("Auto_Prod_CLPThisMonth","Auto_Prod_TrendLast30days","Auto_Prod_ComparisonYTD","Auto_Prod_LPLast7Days","Auto_Prod_ScatterPlotLast30Days");
				...
				*/
				...
				printLog("*******Prod schedule report verification ended!!*******");
		}catch(Exception e) {
			throw e;
		}
	}
